...
@Model.register("EventLSTM")
class EventLSTM(Model):
    ...
    def forward(self,
        ...
        event_type_embed = self.event_type_embedding(event_type)

        # print(len(event_type["tokens"]))
        # print(event_type['tokens'].size()[1])
        ...
        # DA-LSTM need add domain(event_type) information
        if self.domain and event_type['tokens'].size()[1] != 0:
            hn = hn + self.relu(self.W_c(event_type_embed) + self.W_e(hn))
        # [batch_size, 1, embedding_size]
        hn = self.linear(hn)

        # [batch_size, 1] , [batch_size], [batch_size, label_size]
        score, logits, logits_f1 = self.score(hn, e4)
...
if __name__ == '__main__':
    ...
    model = EventLSTM(args, word_embeddings, vocab)
